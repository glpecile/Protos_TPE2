...
unsigned response_read(struct selector_key *key){
    ...
    struct cmd *cmd = NULL;
    if(((response->next_cmd_multiline && response->first_cr && response->first_lf) && dequeue(ATTACHMENT(key)->client.request.cmd_queue,cmd)) > 0){
    ...
    if(bytes_read <= 0){
        ...
        if(response->response_finished || !buffer_can_write(response->res)){
